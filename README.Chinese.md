...
CEACStatusBot Web 是一个自托管的签证状态监控面板，主要支持美国签证 CEAC 状态监控，并新增加拿大 IRCC Portal Alpha 查询。它提供账号注册登录、邮箱验证码、忘记密码、多个签证档案管理、状态历史、邮件通知、管理员后台和生产安全基线，适合本地开发后部署到自己的服务器向公网开放。
...
- 开放注册，注册和忘记密码均通过邮箱验证码完成。新注册账号必须打开正式分条的用户条款和免责声明并明确勾选同意；用户可在个人信息页随时查看同一份条款，登录后打开完整条款会记录为同意当前版本。
- 普通账号可创建 1 个 CEAC 档案，自动查询仍会约每小时运行一次；Issued 后降为每天一次，并在一周后自动停止。普通账号每天可手动立即刷新 1 次，并使用有限的每日业务邮件额度；Premium 账号可创建 5 个档案并拥有高额度查询/邮件额度；管理员不受限制。
- 每个启用档案每小时随机分钟入队查询，Issued 后降为每天一次并在一周后自动停止。
- 登录前页面展示非官方、非盈利、学习研究和授权查询用途说明，不展示赞赏二维码；赞赏入口仅保留在登录后的页面和正向状态 / GTS 通知邮件中。
- 统一用户条款包含跨境查询授权：CEAC/GTS 查询可能会把必要档案字段提交至中国大陆境外的官方或第三方系统。
- CEAC 状态进入 Approved 或 Issued 后，邮件会邀请用户填写 UID/HAL 开启 GTS 护照预约 slot 监控。
- GTS 护照预约监控绑定到签证档案，普通时段约每 1-30 分钟随机轮询一次；自动轮询开关和 slot 变化邮件推送开关相互独立。本功能只检测并通知，不自动预约、不占位、不抢 slot。发现 slot 后会放缓到随机 50-70 分钟一次，用户预约成功后可在站内点击“我已预约，停止监控”。
- 加拿大 IRCC Portal 查询以 Alpha 标记展示，当前仅支持 `https://portal-portail.apps.cic.gc.ca` 这个 IRCC Portal。系统会加密保存用户授权提交的 IRCC Portal 邮箱、密码和 token，用于随机定时查询 submitted applications、application status 和 application messages；任一快照字段变化都会记录历史并发送邮件。该功能未经充分测试，请谨慎使用；未来计划支持 IRCC Portal – New version 和 GCKey。
- `Approved`、`Issued`、Issued 自动停止和 GTS slot 邮件会加入轻量的非盈利赞赏说明，并内嵌与网站相同的赞赏码图片。`Refused` 等负面 CEAC 状态邮件不会放赞赏入口，避免给用户造成不舒服的观感。
- 立即查询会创建手动任务，前端轮询任务状态并刷新档案和时间线；非管理员账号每天有手动查询次数限制。
- 状态无变化不发邮件；状态或 CEAC 更新时间变化时写入历史并发送通知。
- 支持系统默认 SMTP，也支持用户自定义 SMTP。
- 管理员可管理账号等级和 Worker 优先级，查看用户档案概览、queued/running Worker 队列、未来预计进入队列的 CEAC/GTS 自动查询、系统监控日志、安全事件和默认发信配置。
...
新增启用自动监控的签证档案后，系统会立刻排队执行一次初始查询，这次不占用每日手动刷新额度。之后每个启用的签证档案都会保存 `nextCheckAt`。调度器每分钟扫描到期档案，并将自动查询任务写入 SQLite 队列。独立 Worker 消费队列、调用 CEAC 或 IRCC 查询、记录查询日志、更新状态历史并发送邮件通知。

立即查询不会由 Web 进程直接执行爬虫，而是创建任务并返回任务 ID。前端轮询任务状态，完成后刷新档案信息和状态时间线。CEAC 查询失败原因会显示在档案详情页，方便用户判断是需要核对输入信息，还是 CEAC 官网慢、网络波动等临时问题。CEAC 立即查询、GTS 立即查询 slot 和 IRCC 立即查询共用每日手动查询额度：普通账号默认每天 1 次，Premium 默认每天 1000 次，管理员账号不受限制。CEAC/GTS/IRCC 业务邮件也有账号级每日额度：普通账号默认每天 5 封，Premium 默认每天 1000 封。Worker 优先级数值越小越优先；Premium 默认 50，普通账号默认 100，管理员可手动覆盖。

同一个 CEAC 档案连续失败 5 次时，系统会发邮件提醒用户核对办理地点、Application ID 或 Case Number、护照号、姓氏前 5 个字母。任意一次 CEAC 查询成功都会清零连续失败计数。如果连续失败达到 10 次，系统不会立刻停止，而是先降为每天一次继续观察；进入失败降频后 7 天内仍未恢复成功时，系统才会自动停止该档案的 CEAC 自动查询并邮件通知用户。用户仍可手动立即查询。

为减少个人信息长期保存，账号约 15 天没有新的 CEAC 状态历史或 GTS slot 变化历史时，系统会发送删除提醒；提醒后约 15 天仍无新的状态或 slot 动态，即总计约 30 天无动态时，系统会自动删除该账号和相关档案数据。
...
IRCC Portal Alpha 会比较三类快照的稳定指纹：submitted applications 首页数据用于捕捉 ghost update；application status 详情用于比较总状态、资格审查、体检、补充文件、面试、指纹/生物信息、背景调查、最终决定和申请人信息；messages 列表用于发现新消息、消息更新时间和附件名变化。首次快照只记录历史，不作为变化邮件发送；后续任一字段变化都会写入历史并按用户邮件开关发送提醒。如果 IRCC 登录失败、要求 MFA、token 失效且无法刷新，系统会停止该 IRCC 档案自动查询并提示用户重新验证。Alpha 不下载附件、不打开消息详情，避免主动触发可能的已读副作用。

护照预约 slot 监控使用 GTS 官网同源 API：先用 UID/HAL 调用 `https://scheduling-api.gtspremium.com/authenticate` 获取 token，再调用 `/availability7days/` 查询 7 天可用时间。该功能只检测 GTS 返回的可预约时间并发送通知，不自动预约、不占位、不抢 slot，也不保证官网结果完整、实时一致或一定预约成功。UID/HAL、GTS 原始返回和 slot 变化历史都会加密保存。系统会把 GTS 返回归一化为三种状态：`not_eligible` 表示暂不具备预约资格，`no_slot` 表示已可预约但暂无可选时间，`has_slot` 表示发现一个或多个可预约时间。自动轮询由“启用自动监控”控制，slot 变化邮件由“状态更新时发送邮件推送”控制。系统会在 `not_eligible -> no_slot`、`no_slot -> has_slot`、`has_slot -> no_slot`、`no_slot -> not_eligible`、`has_slot -> not_eligible`、以及 `has_slot` 时间列表变化时发邮件。如果监控已经进入过 `no_slot` 或 `has_slot`，后来又回到 `not_eligible`，系统会判断预约资格可能已经结束，发邮件通知并自动停止 GTS 监控。`no_slot` 和 `has_slot` 邮件会包含完整 UID/HAL、GTS 官网入口、站内入口、操作提示和不要转发邮件的安全提醒；系统不使用猜测的 UID/HAL 深链，因为 GTS 官网没有公开可靠的自动填充 URL。slot 邮件会列出可读日期和具体时间，不展示原始 JSON/JWT 摘要。普通时段约每 1-30 分钟随机查询一次；处于 `no_slot` 后按中国时间零点加频：23:59-00:02 约 15 秒一次，23:59:45-00:00:30 核心窗口约 5 秒一次，并明确覆盖 00:00:00 和 00:00:02。一旦确认进入 `no_slot` 或 `has_slot`，系统会自动停止并锁定对应 CEAC 自动查询，只有管理员可以恢复。发现 slot 后 GTS 后续查询放缓到随机 50-70 分钟一次，并且不再参与零点加频；如果后续又变回 `no_slot`，会恢复 `no_slot` 的常规查询节奏。用户也可以在 GTS 官网预约成功后，回到站内点击“我已预约，停止监控”。GTS 查询任务同样由 Worker 消费，触发类型显示为 `passport_slot_manual` 或 `passport_slot_automatic`；队列按 `users.worker_priority ASC, query_jobs.id ASC` 领取，数值越小越优先。管理员后台提供只读 Worker 队列视图，展示 queued/running 任务的当前排序、等待时间和优先级，并基于 `next_check_at` 展示未来预计入队的 CEAC/GTS 自动查询。邮件里的查询时间会按用户最近访问网站时浏览器上报的设备时区显示；尚未保存时区的旧账号暂按中国时间显示。
...
- CEAC 爬虫目标固定为 `https://ceac.state.gov`，GTS slot 查询目标固定为 `https://scheduling-api.gtspremium.com`，IRCC Portal Alpha 查询目标固定为 `https://portal-portail.apps.cic.gc.ca` 和 `https://api.portal-portail.apps.cic.gc.ca`，用户输入不能影响请求 Host 或 URL。
- 统一用户条款中披露跨境查询处理：用户创建档案、启用监控或手动查询时，即授权系统向相关官方或第三方系统提交必要的 CEAC/GTS/IRCC 查询参数。IRCC Portal Alpha 会加密保存用户授权提交的 IRCC Portal 邮箱、密码和 token，请仅在信任本部署环境时使用；测试后建议更改 IRCC Portal 密码。
