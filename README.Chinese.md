...
- GTS 护照预约监控以 Beta 标记展示并绑定到签证档案，普通时段约每 25-35 分钟随机轮询一次；自动轮询开关和 slot 变化邮件推送开关相互独立。本功能只检测并通知，不自动预约、不占位、不抢 slot。发现 slot 后会放缓到约每小时一次，用户预约成功后可在站内点击“我已预约，停止监控”。
...
- 统一用户条款中披露跨境查询处理：用户创建档案、启用监控或手动查询时，即授权系统向相关官方或第三方系统提交必要的 CEAC/GTS 查询参数。
