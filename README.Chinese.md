...
- GTS 护照预约监控绑定到签证档案，普通时段约每 30 分钟轮询一次；自动轮询开关和 slot 变化邮件推送开关相互独立。发现 slot 后会放缓到约每小时一次，用户预约成功后可在站内点击“我已预约，停止监控”。
...
- 管理员可管理账号等级和 Worker 优先级，查看用户档案概览、系统监控日志、安全事件和默认发信配置。
