...
- GTS 护照预约监控绑定到签证档案，默认每 5-10 分钟随机轮询；仅在从无 slot 变为有 slot，或可预约时间变化时发邮件。
- 快速查询会创建手动任务，前端轮询任务状态并刷新档案和时间线。
...
快速查询不会由 Web 进程直接执行爬虫，而是创建 `manual` 任务并返回任务 ID。前端轮询任务状态，完成后刷新档案信息和状态时间线。
...
护照预约 slot 监控使用 GTS 官网同源 API：先用 UID/HAL 调用 `https://scheduling-api.gtspremium.com/authenticate` 获取 token，再调用 `/availability7days/` 查询 7 天可用时间。UID/HAL、GTS 原始返回和 slot 变化历史都会加密保存。系统保存规范化 slot 指纹，只有首次发现可用时间或可用时间列表变化时发送邮件；无 slot 或结果无变化只记录查询日志。GTS 查询任务同样由 Worker 消费，触发类型显示为 `passport_slot_manual` 或 `passport_slot_automatic`。
