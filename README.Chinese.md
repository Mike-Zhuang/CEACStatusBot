...
- 开放注册，注册和忘记密码均通过邮箱验证码完成。新注册账号必须打开正式分条的用户条款和免责声明并明确勾选同意；既有账号不强制补签，但可在个人信息页随时查看同一份条款。
...
同一个 CEAC 档案连续失败 5 次时，系统会发邮件提醒用户核对办理地点、Application ID 或 Case Number、护照号、姓氏前 5 个字母。任意一次 CEAC 查询成功都会清零连续失败计数。如果连续失败达到 10 次，系统会自动停止该档案的 CEAC 自动查询并邮件通知用户；用户仍可手动立即查询。
